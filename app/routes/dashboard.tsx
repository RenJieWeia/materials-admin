...
import { recordConversion, getConversion, getConversions, getAllConversions, getTopUsersByConversion } from "./model/conversion.server";
import { getMaterialUsageStats, getTodayUsageCount, getAllMaterialUsageStats, getMaterialStatusStats, getTopUsersByUsage } from "./model/material.server";
import {
  LineChart,
  Line,
  XAxis,
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const todayConversion = await getConversion(userId, today);
  ...
  const conversions = await getConversions(userId, startDate, endDate);
  const usageStats = await getMaterialUsageStats(user.name, startDate, endDate);

  // Merge data
  const chartData = [];
  for (let d = new Date(startDateObj); d <= new Date(today); d.setDate(d.getDate() + 1)) {
  ...
  if (user.role === 'admin') {
    ...
    const topUsersConversion = await getTopUsersByConversion(startDate, endDate);
  ...
  return { user, todayConversion, today, todayUsageCount, chartData, adminData };
}
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (intent === "record_conversion") {
    ...
    await recordConversion(userId, date, count);
...
export default function Dashboard({ loaderData, actionData }: Route.ComponentProps) {
  const { user, todayConversion, today, todayUsageCount, chartData, adminData } = loaderData;
      ...
      <div className="p-4 container mx-auto text-gray-800 dark:text-white">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-300">全平台近7日趋势</h2>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={adminData.chartData}>
                  ...
                  />
                  <Legend />
                  <Bar dataKey="conversion" name="总转化数" fill="#3B82F6" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="usage" name="总账号使用" fill="#10B981" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-300">账号状态分布</h2>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-300">转化榜 Top 5 (近7日)</h2>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart layout="vertical" data={adminData.topUsersConversion}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.1} />
                  <XAxis type="number" stroke="#6B7280" fontSize={12} />
                  <YAxis dataKey="user" type="category" stroke="#6B7280" fontSize={12} width={80} />
                  <Tooltip 
                    contentStyle={{ backgroundColor: '#1F2937', borderColor: '#374151', color: '#F3F4F6' }}
                    itemStyle={{ color: '#F3F4F6' }}
                  />
                  <Bar dataKey="count" name="转化数" fill="#8B5CF6" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          {/* Top Users by Usage */}
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-300">勤奋榜 Top 5 (近7日)</h2>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart layout="vertical" data={adminData.topUsersUsage}>
                  ...
                  <Tooltip 
                    ...
                    itemStyle={{ color: '#F3F4F6' }}
                  />
                  <Bar dataKey="count" name="账号使用数" fill="#F59E0B" radius={[0, 4, 4, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
  ...
  return (
    <div className="p-4 container mx-auto text-gray-800 dark:text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        {/* Conversion Card */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">今日转化</h2>
        ...
        {/* Today Usage Card */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-semibold mb-2 text-gray-700 dark:text-gray-300">今日账号使用</h2>
          <div className="text-3xl font-bold text-green-600 dark:text-green-400 mb-4">
            {todayUsageCount}
          </div>
          <div className="text-sm text-gray-500 dark:text-gray-400">
            今日转化率: {todayUsageCount > 0 ? ((todayConversion?.count || 0) / todayUsageCount * 100).toFixed(1) : 0}%
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Conversion Trend */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-300">近7日转化趋势</h2>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                />
                <Legend />
                <Bar dataKey="conversion" name="转化数" fill="#3B82F6" radius={[4, 4, 0, 0]} />
                <Bar dataKey="usage" name="账号使用数" fill="#10B981" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-300">近7日转化率趋势</h2>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                />
                <Legend />
                <Line type="monotone" dataKey="rate" name="转化率" stroke="#8B5CF6" strokeWidth={2} dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            <div className="relative inline-block transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:align-middle">
              <div className="bg-white dark:bg-gray-800 px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                ...
                <Form method="post" ref={formRef}>
                  <input type="hidden" name="intent" value="record_conversion" />
                  <div className="mb-4">
